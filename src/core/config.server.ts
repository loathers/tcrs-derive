...
export function resolveBatchConfig(o: ResolveOptions = {}): BatchConfig {
	...
	const javaOpts = (env.JAVA_OPTS ?? "").split(/\s+/).filter((s) => s !== "");
	...
	const cfg: BatchConfig = {
		...
		loginTimeoutMs:
			num(env.LOGIN_TIMEOUT, DEFAULTS.loginTimeoutMs / 1000) * 1000,
		timeoutMs: num(env.TIMEOUT, DEFAULTS.timeoutMs / 1000) * 1000,
		retryBackoffMs:
			num(env.RETRY_BACKOFF, DEFAULTS.retryBackoffMs / 1000) * 1000,
