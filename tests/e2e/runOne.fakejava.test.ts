...
import { runOne, type RunOneOptions } from "#core/runOne.server";
...
const AT_BLENDER = permutationByUser("at_blender")!;
...
function harness(
  ...
  return {
    ...
    find: (type) =>
      events.filter((e) => e.type === type) as never,
  };
}
...
describe("the happy path", () => {
  ...
  it("does not decide on 'exit' before stdout is drained", async () => {
    ...
    expect(progress.at(-1)!.done).toBe(12001);
  });
...
describe("partial output is discarded, not published", () => {
  ...
  it("retries after an incomplete attempt and can then succeed", async () => {
    ...
    expect(h.find("perm:retryWait")[0]!.nextAttempt).toBe(2);
  ...
  it("rejects files with no progress ever reported", async () => {
    const h = harness(["--fake-fixture=not-in-tcrs", "--fake-perm=Accordion_Thief_Blender"], {
      maxAttempts: 1,
    });
    const r = await runOne(h.opts);
    expect(r.ok).toBe(false);
  ...
  it("never retries not-in-tcrs even with attempts remaining", async () => {
    const h = harness(
      ["--fake-fixture=not-in-tcrs", "--fake-files=0"],
      { maxAttempts: 3 },
    );
    const r = await runOne(h.opts);
    expect(r.reason).toBe("not-in-tcrs");
...
describe("watchdogs", () => {
  ...
  it("kills a child that ignores SIGTERM", async () => {
    ...
    expect(exited[0]!.signal).toBe("SIGKILL");
  }, 30_000);
  ...
  it("reaps descendants by killing the process group", async () => {
    ...
    expect(Number.isFinite(pid), `no grandchild pid in output: ${log}`).toBe(true);
    ...
    expect(alive, `grandchild ${pid} survived the process-group kill`).toBe(false);
  }, 30_000);
});
...
describe("work dir seeding", () => {
  ...
  it("warns but continues when the template cannot be copied", async () => {
    ...
    const r = await runOne(h.opts);
    expect(h.find("warn").some((w) => w.message.includes("template"))).toBe(true);
    expect(r.ok).toBe(true);
  });
});
...
describe("the TCRS output location", () => {
  it("collects from data/TCRS/, as r29183+ writes it", async () => {
    ...
    expect(h.find("perm:wrote").map((e) => e.file).sort()).toEqual(
      [...AT_BLENDER.files].sort(),
    );
  }, 30_000);
