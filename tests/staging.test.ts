...
const SC = present(permutationByUser("sc_mongoose"));
...
describe("indexFiles and writeSums", () => {
	it("hashes and measures the files that exist", async () => {
		...
		expect(present(entries[0]).bytes).toBe(5);
		expect(present(entries[0]).sha256).toBe(
			"2cf24dba5fb0a30e26e83b2ac5b9e29e1b161e5c1fa7425e73043362938b9824",
		);
		expect(present(entries[0]).sourceRunId).toBe("run-1");
	});
	...
	it("writes SHA256SUMS.txt in sha256sum -c format", async () => {
		...
		expect(sums).toBe(`${present(entries[0]).sha256}  ${AT.files[0]}\n`);
	});
});
...
describe("publishRun", () => {
	...
	it("marks carried-forward files as coming from the earlier run", async () => {
		...
		expect(present(byName.get(AT.files[0])).sourceRunId).toBe("run-1");
		expect(present(byName.get(AT.files[1])).sourceRunId).toBe("run-2");
	});
	...
	it("publishes atomically: manifest, zip and symlink all land together", async () => {
		...
		for (const name of AT.files)
			writeFileSync(join(staging.dataDir, name), "x");
		...
		expect(present(await readCurrentManifest(data)).id).toBe("run-1");
	...
	it("keeps files no permutation in this run was even asked to produce", async () => {
		...
		const byName = new Map(published.entries.map((e) => [e.name, e]));
		expect(present(byName.get(AT.files[0])).sourceRunId).toBe("run-2");
		expect(present(byName.get(SC.files[0])).sourceRunId).toBe("run-1");
		expect(
			readFileSync(join(data, "current", "data", SC.files[0]), "utf8"),
		).toBe("from run 1");
	});
